...
//
// forked from design-sync lib/source-kit.mjs — this repo's UI components are
// all `export default function Name()`. The stock synth-entry writer emits
// `export * from <path>` for every file, which silently drops default exports
// (ESM `export *` never forwards a module's default), so none of them would
// reach window.<GLOBAL>.<Name> in the actual functional bundle even though
// deriveComponentsFromSrc (unchanged below) correctly discovers their names.
// This fork adds a `export { default as Name } from <path>` line alongside
// the original `export *` line whenever a file's default export name can be
// determined, leaving named-export files (and everything else) unchanged.
...
// Best-effort detection of a file's default-exported declaration name, so the
// synth entry can re-export it under that name (`export *` alone can't).
function defaultExportName(file) {
  const text = readText(file);
  let m = /export\s+default\s+function\s+([A-Z][A-Za-z0-9]*)/.exec(text)
    || /export\s+default\s+class\s+([A-Z][A-Za-z0-9]*)/.exec(text);
  if (m) return m[1];
  const idm = /export\s+default\s+([A-Z][A-Za-z0-9]*)\s*;/.exec(text);
  if (idm && new RegExp(`\\b(?:function|class|const)\\s+${idm[1]}\\b`).test(text)) return idm[1];
  return null;
}
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    writeFileSync(entry, comps.map((p) => {
      const lines = [`export * from ${JSON.stringify(p)};`];
      const defName = defaultExportName(p);
      if (defName) lines.push(`export { default as ${defName} } from ${JSON.stringify(p)};`);
      return lines.join('\n');
    }).join('\n') + '\n');
    synthEntry = true;
    console.error(

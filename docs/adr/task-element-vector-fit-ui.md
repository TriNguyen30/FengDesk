...
| 3 Chi tiết sản phẩm                      | ScoreBadge + SpaceTabs + EmptyState, rồi **một hộp** `ScoreWaterfall aside=… lead=…` (trái = `ElementRadarChart` + `OccupationDirectionPanel`; phải = `lead` (`RoomNeedCard`) + waterfall + "Đã xem xét các yếu tố" bung inline; **cột phải cao bằng cột trái, dư thì cuộn dọc trong cột với snap đỉnh/đáy**), rồi hàng `[InfoCardTrio | ConflictResolutionBanner]` DƯỚI hộp (banner "ưu tiên hành X để cân bằng" chỉ khi có xung, co theo nội dung; không có thì trio trải hết). `PersonalFitPanel` cùng khung với `lead=NeedCard`. Mọi popover trong cột cuộn dùng `useAnchoredPopover` (`position: fixed`) để không bị overflow xén. Trạng thái chờ: `FitLoadingBanner` (bot + spinner + thanh chạy, cùng kiểu `WorkspaceIntakeProgress`) thay khối xám nhấp nháy; dữ liệu về thì các khối mở lần lượt (`revealVariants` + `Reveal`, stagger 70 ms). `cautionFacts` tô đỏ chỉ khi chạm bản mệnh (`cautionTone`), còn lại vàng (20/09/2026: bỏ `ElementBars`, `PersonalWeightControls`, `InfoCardTrio`, `SummaryLine` — bars lặp radar, slider gây hiểu nhầm trọng số là thứ user chỉnh) | —                                                                                     | fit sản phẩm × phòng           | `fit`              |

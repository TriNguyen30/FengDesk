...
| 3 Chi tiết sản phẩm                      | ScoreBadge + SpaceTabs + EmptyState, rồi 2 cột **cùng bố cục với `PersonalFitPanel`**: trái = `ElementRadarChart` + `OccupationDirectionPanel`, phải = `RoomNeedCard` + `ScoreWaterfall` (20/09/2026: bỏ `ElementBars`, `PersonalWeightControls`, `InfoCardTrio`, `SummaryLine` — bars lặp radar, slider gây hiểu nhầm trọng số là thứ user chỉnh) | —                                                                                     | fit sản phẩm × phòng           | `fit`              |

...
import ShopReturnsView from "@/features/shop/components/ShopReturnsView";
  ...
  useEffect(() => {
    ...
    const fetchUserStores = async () => {
      ...
      try {
        ...
        console.error("Failed to load active stores for order return page:", err);
      } finally {
        if (isMounted) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">Quản lý Yêu cầu Trả hàng</h1>
          <p className="text-gray-500 mt-1 text-sm">
            Xem và xử lý các yêu cầu đổi trả / hoàn tiền thuộc chi nhánh bạn quản lý.
          </p>
        </div>
      ...
      {loadingStores ? (
        ...
        <ShopReturnsView key={selectedStoreId} storeId={selectedStoreId} />
      ) : null}
    </div>
  );
}

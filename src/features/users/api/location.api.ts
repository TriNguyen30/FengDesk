...
export const getDistrictsByProvinceId = async (provinceId: string): Promise<District[]> => {
    const response = await fetchHttpClient.get<ApiResponse<District[]>>(`/locations/provinces/${provinceId}/districts`);
    return response.data.data;
};

export const getWardsByDistrictId = async (districtId: string): Promise<Ward[]> => {
    const response = await fetchHttpClient.get<ApiResponse<Ward[]>>(`/locations/districts/${districtId}/wards`);
    return response.data.data;
};
...
import { Workspace, WorkspaceType, Style, CreateWorkspaceDto, UpdateWorkspaceDto } from "../types/workspace";
...
export const setDefaultWorkspace = async (id: string): Promise<Workspace> => {
  const response = await fetchHttpClient.patch<ApiResponse<Workspace>>(`/workspace/${id}/set-default`);
  return response.data.data;
};

...
interface RoomNeedCardProps {
  ...
  /** Gợi ý hướng đặt của BE — null khi nhóm vật không xét hướng. */
 ...
 */
export default function RoomNeedCard({ workspaceName, gap, breakdown, placementHint, evidenceCount }: RoomNeedCardProps) {
  const [open, setOpen] = useState(false);

  const byElement = new Map(gap.map((r) => [r.element, r]));
  const ranked = ELEMENT_ORDER
    .map((e) => ({ element: e, gap: byElement.get(e)?.gap ?? 0, ideal: byElement.get(e)?.adjustedIdeal ?? 0, current: byElement.get(e)?.current ?? 0 }))
  ...
  return (
    <div
      className="relative rounded-xl border border-primary/30 bg-primary/[0.06] px-3 py-2.5 text-xs outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
      tabIndex={0}
    ...
    >
      <div className="flex items-center gap-1.5 font-semibold text-[#4a6a2c]">
        ...
        <span className="ml-auto shrink-0 text-[10px] font-medium text-[#5f7d3c]">
          {evidenceCount === 0 ? "suy từ loại phòng" : `${evidenceCount} nguồn bạn đã khai`}
        </span>
      </div>

      <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
        {lacking.length === 0 && surplus.length === 0 && (
          <span className="text-[11px] text-gray-600">Phòng đang cân bằng — không hành nào thiếu hay thừa rõ rệt.</span>
        )}
        {lacking.map((r) => (
          <span key={r.element} className="inline-flex items-center gap-1 rounded-full bg-white/80 px-2 py-0.5 text-[11px] font-medium text-gray-700">
            <span className="inline-block h-2 w-2 rounded-full" style={{ backgroundColor: elementColor(r.element) }} />
            {elementVi(r.element)} +{Math.round(r.gap * 100)}%
          </span>
        ))}
        {surplus.length > 0 && (
          <>
            <span className="ml-1 text-[10px] uppercase tracking-wide text-gray-400">đang thừa</span>
            {surplus.map((r) => (
              <span
                key={r.element}
      ...
      <p className="mt-1.5 text-[11px] leading-snug text-gray-600">
        {offReason ? (
          ...
          <>
            ...
            {wp > 0 && <> · <b>{Math.round(wp * 100)}% bản mệnh</b></>}
            {wo > 0 && <> · <b>{Math.round(wo * 100)}% nghề</b></>}
          </>
        )}
      </p>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute left-0 top-full z-30 mt-1 w-[300px] max-w-[calc(100vw-2rem)] rounded-xl border border-primary/50 p-3 text-[11px] text-slate-700 shadow-lg backdrop-blur-[3px]"
            style={{ background: "rgba(255,255,255,0.97)" }}
          >
            <p className="mb-1.5 font-semibold text-slate-900">Các con số này từ đâu?</p>
            <p className="leading-snug">
              <b>Mức lý tưởng</b> lấy theo loại phòng và mục đích sử dụng; <b>hiện tại</b> gộp từ nền phòng,
              màu sắc/vật liệu bạn khai, đồ đã đặt và bản mệnh chủ phòng. Phòng <b>cần</b> = lý tưởng − hiện tại.
            </p>
                ...
                <span key={r.element} className="contents">
                  ...
                  <span
                    className={`text-right font-medium tabular-nums ${
                      r.gap > GAP_THRESHOLD ? "text-[#4a6a2c]" : r.gap < -GAP_THRESHOLD ? "text-[#b3261e]" : "text-slate-400"
                    }`}
                  >
                    {r.gap > GAP_THRESHOLD ? "thiếu" : r.gap < -GAP_THRESHOLD ? "thừa" : "ổn"} {Math.abs(Math.round(r.gap * 100))}%
                  </span>
                </span>
              ))}
            </div>
            {breakdown?.personalWeight && !offReason && (
              <p className="mt-2 leading-snug">
                <b>Trọng số bản mệnh {Math.round(wp * 100)}%</b>: {breakdown.personalWeight.reasonVi}
              </p>
            )}
          </motion.div>
        )}
...
function offReasonText(reason: NonNullable<ReturnType<typeof personalAxisOffReason>>) {
  switch (reason) {
    case "public-space":
      return <><b>Không gian chung</b> — điểm chỉ tính theo nhu cầu của phòng, không neo vào bản mệnh của riêng ai.</>;
    case "no-birthdate":
      return (
        <>
          Chưa có ngày sinh trong hồ sơ — điểm chỉ dựa trên nhu cầu của phòng.{" "}
          <a href="/profile" className="underline underline-offset-2">Thêm ngày sinh</a> để nhận gợi ý hợp bản mệnh.
        </>
      );
    default:

...
export function useProductFit(productId?: string, workspaceProfileId?: string) {
  const query = useQuery({
    ...
    queryFn: () => {
      if (!productId || !workspaceProfileId) throw new Error("Missing productId/workspaceProfileId");
...
export function useProductFitAcrossWorkspaces(productId: string | undefined, workspaceIds: string[]) {
